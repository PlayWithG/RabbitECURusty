-- ============================================================
-- DSG DQ250 Standalone v2 — rusEFI Lua Script (Universal)
-- ============================================================
-- Versión mejorada con estimador de torque multi-método
...
--
-- MEJORAS v2:
-- - Estimador de torque configurable (4 métodos + auto-selección)
-- - Corrección por temperatura IAT (densidad del aire)
-- - MAF directo (más preciso si sensor disponible)
-- - Tabla 3D TunerStudio (configurable sin recompilar)
-- - MAP×RPM (fallback inteligente)
-- - Lineal MAP (fallback final universal)
--
...
-- ============================================================
-- ESTIMADOR DE TORQUE v2 — configuración ampliada
-- ============================================================

-- Método de estimación de torque:
--   0 = auto (MAF → tabla3d → MAP×RPM → lineal, primera disponible)
--   1 = lineal MAP    (original, fallback universal)
--   2 = MAP × factor RPM
--   3 = tabla 3D TunerStudio (tabla Lua #1: eje X=RPM, Y=MAP)
--   4 = MAF directo (más preciso con sensor MAF calibrado)
local TORQUE_METHOD    = 0

-- Límites RPM para factor de corrección de RPM
local RPM_IDLE         = 800     -- RPM de ralentí
local RPM_PEAK_TQ      = 3500    -- RPM de torque máximo del motor
local RPM_REDLINE      = 7000    -- RPM de corte

-- Factor de torque normalizado en extremos de RPM (0.0-1.0)
-- (el pico de torque = 1.0 siempre, estos son los valores en los extremos)
local RPM_FACTOR_IDLE  = 0.65    -- fracción del pico de torque en ralentí
local RPM_FACTOR_TOP   = 0.85    -- fracción del pico de torque en redline

-- Corrección por temperatura IAT (densidad del aire)
local IAT_CORRECTION   = true    -- true = aplicar corrección IAT
local IAT_REF          = 25.0    -- °C de referencia (densidad nominal)
...
-- Seguimiento del método activo de torque (para monitoreo)
local lastTqMethod = 0
...
-- ============================================================
-- ESTIMADOR DE TORQUE MULTI-MÉTODO v2
-- ============================================================

-- ---- Interpolación clampeada (NO extrapola fuera del rango) ----
-- Equivalente al patrón de launch_control.lua en rusReference
local function clampedInterp(x1, y1, x2, y2, x)
    if x <= x1 then return y1 end
    if x >= x2 then return y2 end
    return interpolate(x1, y1, x2, y2, x)
end

-- ---- Factor de corrección por RPM (0.0-1.0) ----
-- Curva: RPM_IDLE→RPM_FACTOR_IDLE → RPM_PEAK_TQ→1.0 → RPM_REDLINE→RPM_FACTOR_TOP
local function rpmTqFactor(rpm)
    if rpm == nil or rpm < RPM_IDLE then return RPM_FACTOR_IDLE end
    if rpm <= RPM_PEAK_TQ then
        return clampedInterp(RPM_IDLE, RPM_FACTOR_IDLE, RPM_PEAK_TQ, 1.0, rpm)
    else
        return clampedInterp(RPM_PEAK_TQ, 1.0, RPM_REDLINE, RPM_FACTOR_TOP, rpm)
    end
end

-- ---- Factor de corrección IAT (densidad del aire) ----
-- Ley termodinámica: más caliente = menos denso = menos torque
-- Factor = 1.0 cuando IAT == IAT_REF
local function iatFactor(iat)
    if not IAT_CORRECTION or iat == nil then return 1.0 end
    return (273.0 + IAT_REF) / (273.0 + iat)
end

-- ---- MÉTODO 1: Lineal MAP (fallback universal) ----
local function calcTorqueLinear(map)
...
-- ---- MÉTODO 2: MAP × factor RPM ----
local function calcTorqueMapRpm(rpm, map)
    local base = calcTorqueLinear(map)
    if base <= 0 then return 0 end
    return math.min(255, math.floor(base * rpmTqFactor(rpm)))
end

-- ---- MÉTODO 3: Tabla 3D TunerStudio ----
-- Usa tabla Lua #1: eje X = RPM, eje Y = MAP (kPa)
-- La tabla debe devolver valores 0-255 según calibración en TunerStudio
local function calcTorqueTable(rpm, map)
    local v = table3d(1, rpm or 0, map or 0)
    if v == nil then return nil end  -- tabla no configurada
    return math.min(255, math.max(0, math.floor(v)))
end

-- ---- MÉTODO 4: MAF directo ----
-- mafMassRef: masa de aire por ciclo a plena carga (g/ciclo)
-- Se calcula automáticamente desde displacement en initMAFRef()
local mafMassRef = 0.0

local function initMAFRef()
    local dispCC = getCalibration("displacement") or 1998
    -- Referencia: VE=85% a plena carga
    -- Fórmula: displacement_cc × 0.00051 g/ciclo (validado para turbo)
    mafMassRef = dispCC * 0.00051
end

-- maf en g/s, rpm en RPM → tq 0-255
local function calcTorqueMAF(rpm, maf)
    if maf == nil or maf <= 0 or rpm == nil or rpm < 100 then return nil end
    if mafMassRef <= 0 then return nil end
    -- masa por ciclo = maf_g_s × 120 / rpm  (120 = 60s × 2 rev/ciclo)
    local massPerCycle = maf * 120.0 / rpm
    local ratio = massPerCycle / mafMassRef
    return math.min(255, math.floor(ratio * 255))
end

-- ---- ESTIMADOR PRINCIPAL: auto-selección de método ----
-- Aplica corrección IAT al resultado final (si habilitada)
local function calcTorqueEst(rpm, map, maf, iat)
    local tq = nil
    local method = 0

    if TORQUE_METHOD == 4 then
        -- Método forzado: MAF
        tq = calcTorqueMAF(rpm, maf)
        method = 4
    elseif TORQUE_METHOD == 3 then
        -- Método forzado: Tabla 3D
        tq = calcTorqueTable(rpm, map)
        method = 3
    elseif TORQUE_METHOD == 2 then
        -- Método forzado: MAP×RPM
        tq = calcTorqueMapRpm(rpm, map)
        method = 2
    elseif TORQUE_METHOD == 1 then
        -- Método forzado: Lineal MAP
        tq = calcTorqueLinear(map)
        method = 1
    else
        -- TORQUE_METHOD == 0: auto, primera disponible
        tq = calcTorqueMAF(rpm, maf)
        if tq ~= nil then
            method = 4
        else
            local tqT = calcTorqueTable(rpm, map)
            if tqT ~= nil and tqT > 1 then
                tq = tqT
                method = 3
            end
        end
    ...
    -- Fallback final: nunca devolver nil
    if tq == nil then
        tq = calcTorqueLinear(map)
        method = 1
    end

    -- Guardar método activo para monitoreo
    lastTqMethod = method

    -- Aplicar corrección IAT
    if IAT_CORRECTION and iat ~= nil then
        tq = math.floor(tq * iatFactor(iat))
    end

    return math.max(0, math.min(255, tq))
end
...
-- ============================================================
-- INICIALIZACIÓN — llamar antes de setTickRate()
-- ============================================================
initMAFRef()
...
function onTick()
    ...
    local maf   = getSensor("Maf")                     -- g/s (nil si no hay sensor)
    local iat   = getSensor("Iat") or IAT_REF          -- °C
